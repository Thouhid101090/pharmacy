                ...
                <form action="{{ route('purchases.index') }}" method="GET">
                    <div class="input-group">
                        <input type="text" name="search" class="form-control border-0 shadow-none"
                            placeholder="Search by medicine or supplier..." value="{{ request('search') }}">
                        ...
                        @foreach ($purchases as $purchase)
                            <tr>
                                ...
                                <td class="text-center"><span
                                        class="badge bg-light text-dark border">{{ $purchase->quantity }}</span></td>
                                <td class="fw-bold">{{ number_format($purchase->total_amount, 2) }}</td>
                                <td>
                                    @if ($purchase->expiry_date)
                                        <span
                                            class="small {{ \Carbon\Carbon::parse($purchase->expiry_date)->isPast() ? 'text-danger fw-bold' : '' }}">
            ...
            <div class="d-md-none">
                @foreach ($purchases as $purchase)
                    <div class="p-3 border-bottom">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <div>
                                <span class="badge bg-soft-primary text-primary mb-1"
                                    style="background-color: #e7f1ff;">{{ $purchase->invoice_no }}</span>
                        ...
                        <div class="d-flex justify-content-between align-items-center bg-light p-2 rounded small mb-2">
                            <span>Qty: <strong>{{ $purchase->quantity }}</strong></span>
                            <span>Exp: <strong
                                    class="{{ \Carbon\Carbon::parse($purchase->expiry_date)->isPast() ? 'text-danger' : '' }}">{{ $purchase->expiry_date ?? 'N/A' }}</strong></span>
                        </div>

                        {{-- Added Actions for Mobile --}}
                        <div class="d-flex gap-2">
                            ...
                            <form action="{{ route('purchases.destroy', $purchase->id) }}" method="POST"
                                onsubmit="return confirm('Delete this record?');" class="flex-grow-1">
                                @csrf
                                @method('DELETE')
